...
val_writer_write_obj(struct val_writer *self, struct val_obj *obj) {
    ...
    while (iterator_has_next(&it)) {
        ...
        switch (val->kind) {
            case VAL_KIND_STR: {
                CHECK_EQUAL(JSON_WRITER_STATUS_OK,
                            json_writer_obj_str(&self->writer, JSON_KEY(prop->field->ident), AS_STR(val)));
                break;
            }
            case VAL_KIND_OBJ: {
                CHECK_EQUAL(JSON_WRITER_STATUS_OK,
                            json_writer_obj_start_obj(&self->writer, JSON_KEY(prop->field->ident)));
                val_writer_write_obj(self, AS_OBJ(val));
