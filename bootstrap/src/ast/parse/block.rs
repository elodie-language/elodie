...
impl Parser {
    pub(crate) fn parse_block(&mut self) -> crate::ast::parse::Result<BlockNode> {
        self.consume_operator(OperatorToken::OpenCurly)?;

        let mut nodes = Vec::new();
        loop {
            self.consume_if(TokenKind::Separator(NewLine))?;
            if self.current()?.is_operator(OperatorToken::CloseCurly) {
                break;
            }
            nodes.push(self.parse_node(Precedence::None)?);
        }
        self.consume_operator(OperatorToken::CloseCurly)?;
        Ok(BlockNode { nodes })
    }
}
...
mod tests {
    ...
    use crate::ast::token::{LiteralToken, test_token, test_token_with_offset, TokenKind};
