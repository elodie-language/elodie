...
impl<'a> Generator<'a> {
    ...
    pub(crate) fn generate_node(&mut self, node: &parse::Node) -> Result<node::Node> {
        match node {
            ...
            parse::Node::ExternalFunctionDeclaration(node) => self.generate_declare_external_function(node),
            parse::Node::FunctionDeclaration(declaration_node) => Ok(self.generate_declare_function(declaration_node)?),
            parse::Node::PackageDeclaration(declaration_node) => Ok(self.generate_declare_package(declaration_node)?),
            parse::Node::Identifier(identifier_node) => Ok(self.generate_identifier(identifier_node)?),
