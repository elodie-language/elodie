...
use crate::frontend::ast::{AstBlockNode, AstBreakLoopNode, AstContinueLoopNode, AstIfNode, AstLoopNode, AstNode, AstTreeNode, Generator, SPAN_NOT_IMPLEMENTED};

impl<'a> Generator<'a> {
    pub(crate) fn generate_break(&mut self, node: &parse::BreakNode) -> ast::Result<AstTreeNode> {
        if node.result.is_none() {
            Ok(AstTreeNode::new(AstNode::BreakLoop(AstBreakLoopNode {
                node: None,
            }), SPAN_NOT_IMPLEMENTED.clone()))
        } else {
            let node = Some(Rc::new(self.generate_node(node.result.as_ref().unwrap())?));
            Ok(AstTreeNode::new(AstNode::BreakLoop(AstBreakLoopNode { node }), SPAN_NOT_IMPLEMENTED.clone()))
        }
    }

    pub(crate) fn generate_continue(&mut self, _node: &parse::ContinueNode) -> ast::Result<AstTreeNode> {
        Ok(AstTreeNode::new(Node::ContinueLoop(AstContinueLoopNode {}), SPAN_NOT_IMPLEMENTED.clone()))
    }
    ...
    pub(crate) fn generate_loop(&mut self, node: &parse::LoopNode) -> ast::Result<AstTreeNode> {
        ...
        Ok(AstTreeNode::new(Node::Loop(AstLoopNode { nodes }), SPAN_NOT_IMPLEMENTED.clone()))
    }
    ...
    pub(crate) fn generate_if(&mut self, node: &parse::IfNode) -> ast::Result<AstTreeNode> {
        ...
        Ok(AstTreeNode::new(Node::If(AstIfNode {
            condition,
