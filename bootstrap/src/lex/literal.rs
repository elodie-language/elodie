...
impl Lexer<'_> {
    ...
    pub(crate) fn consume_string(&mut self) -> crate::lex::Result<Token> {
    ...
    pub(crate) fn consume_number(&mut self) -> crate::lex::Result<Token> {
        let start = self.position();
        let mut text = String::from("");
        let next = String::from(self.consume_next()?);
    ...
    pub(crate) fn consume_bool(&mut self) -> crate::lex::Result<Token> {
        let start = self.position();
        let next = self.consume_next()?;
        if next == 't' {
