...
use crate::ir::analyse::{DeclareVariableNode, Node};
use crate::ir::analyse::infer::Inference;
use crate::ir::symbol::SymbolName;

impl<'a> Inference<'a> {
    pub(crate) fn infer_declare_variable(&mut self, parsed_node: Rc<ast::DeclareVariableNode>) -> crate::ir::analyse::Result<Node> {
        let symbol = self.register_variable(SymbolName::from(&parsed_node.identifier));

        let mut node = self.infer_node(parsed_node.value.clone())?;
...
mod tests {
    ...
    // use crate::ir::analyse::node::Node::DeclareVariable;
